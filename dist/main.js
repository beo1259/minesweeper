...
function setStylesOnHighScoresModelAction(isShowing) {
    ...
    for (const el of allElements) {
        if (el === modal || modal.contains(el) || el.contains(modal) || el.tagName === 'BODY' || el.tagName === 'HTML' || el.tagName === 'HEAD') {
            continue;
        }
        const htmlEl = el;
        if (isShowing) {
