...
document.addEventListener('keyup', (e) => {
    ...
    if (k === "f") handleCellMain(x, y);
    else if (k === "g") handleCellSecondary(x, y)
})
...
document.addEventListener('mouseup', (e) => {
    ...
    if (e.button === 0) handleCellMain(x, y);
    else if (e.button === 2) handleCellSecondary(x, y);
})

export function handleCellMain(x: number, y: number): void {
    const cell = board[x][y];
    if (cell.cellState === CellState.Flagged || cell.isOpen) return;
...
export function handleCellSecondary(x: number, y: number): void {
    let updatedCell = new Cell(x, y, 0, CellState.Flagged, true);
    updateCell(x, y, updatedCell)
}

function getCellByCoords(x: number, y: number) {
    return board[x][y];
}

function updateCell(x: number, y: number, updatedCell: Cell) {
    board[x][y] = updatedCell;
    let htmlCell = document.getElementById(`cell_${x}_${y}`);
    htmlCell!.className = getCellClassName(updatedCell);
}

function initBoard(xSize: number, ySize: number): void {
    initBoardMatrix(xSize, ySize);
    drawBoard(xSize, ySize);
}

function initBoardMatrix(xSize: number, ySize: number): void {
...
function getMineCoordinatesOnInit(xSize: number, ySize: number) {
...
function drawBoard(xSize: number, ySize: number): void {
...
function getMineCountByBoardSize(xSize: number, ySize: number) {
    if (xSize === 8 && ySize === 8) return 10;
    if (xSize === 16 && ySize === 16) return 40;
    if (xSize === 30 && ySize === 16) return 99;
    else throw new Error("Invalid board size");
}
